...
import com.basics.framework.core.annotations.Union;
import com.basics.framework.core.models.StateList;
                ...
                (singProperty) -> {
                    try {
                        ...
                        if (union.forLimitQuery() && (pValue = extractValueFormReadMethod(sourceObj, property.getReadMethod())) != null
                                && org.apache.commons.lang3.StringUtils.isNotEmpty(String.valueOf(pValue))) {
                            //如果是为了去其他服务限制查询的属性
                            //获取属性值,如果有值，记录该值
    ...
    private static void processUnio(Object targetObj, List igls, PropertyDescriptor targetProperty, Map<String, ClassWithSourcePair> mappedTargets, Union union) {
        ...
        if (union != null) {
            ...
            if (findKey == null && !ArrayUtils.isEmpty(union.bind())) {
                ...
                while (findKey == null && current < bindlenth && !org.apache.commons.lang3.StringUtils.equals(binds[current], mergeKey)/*剔除已经处理过的mergeKey*/) {
