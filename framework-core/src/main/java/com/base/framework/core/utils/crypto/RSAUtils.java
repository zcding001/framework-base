package com.base.framework.core.utils.crypto;
...
public class RSAUtils {
	...
	public static byte[] encryptByPublicKey(byte[] data, String  publicKey)
		...
		return Base64.getEncoder().encode(bufRes);
	}
	...
	public static void main(String[] args) throws Exception {
		...
		byte [] buf = encryptByPublicKey(Base64.getEncoder().encode(str.getBytes()), publicKey);
		
		System.out.println(new String(buf));
		
		byte [] res = decryptByPrivateKey(buf, privateKey);
		
		System.out.println(new String(Base64.getDecoder().decode(res)));
	}
}
